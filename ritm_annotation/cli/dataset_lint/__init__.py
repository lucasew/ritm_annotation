...
COMMAND_DESCRIPTION = _("Looks for common problems in datasets in mask form that are used for finetuning and generated by the annotator")  # noqa: E501
        ...
        def handle_one(item):
            ...
            if args.images is not None:
                ...
                if not image_file.exists():
                    logger.warning(_("'{image_file}': Image file doesn't exist").format(image_file=image_file))
                else:
            ...
            for mask in item.iterdir():
                ...
                if item_img is not None:
                    ...
                    if wi != wm:
                        logger.error(
                            _("'{mask}': First dimension doesn't match for image and mask").format(mask=mask)  # noqa:E501
                        )
                    if hi != hm:
                        logger.error(
                            _("'{mask}': Second dimension doesn't match for image and mask").format(mask=mask)  # noqa:E501
                        )
